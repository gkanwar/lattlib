...
def run_hmc(L, n_step, n_skip, n_therm, tau, n_leap, action, cfg):
    ...
    pure_bosonic_action = PureBosonicAction(beta=action.beta)
...
if __name__ == "__main__":
    ...
    if args.type == "pure_bosonic":
        assert(args.beta is not None)
        action = PureBosonicAction(args.beta)
    ...
    # do the thing!
    cfgs = run_hmc(L, tot_steps, args.n_skip, args.n_therm,
                   args.tau, args.n_leap, action, cfg)

    # write stuff out
    prefix = 'thirring_{:s}_N{:d}_skip{:d}_therm{:d}_{:d}_{:d}{:s}'.format(
